...
async function approvePendingTaskBtn(taskId) {
...
async function rejectPendingTaskBtn(taskId) {
  const notes = prompt('Motivo da rejeição (recomendado):');
  if (notes === null) return; // Cancelou
  
  if (!notes.trim()) {
    if (!confirm('Deseja realmente rejeitar sem informar um motivo?')) {
      return;
    }
  }
...
window.approvePendingTaskBtn = approvePendingTaskBtn;
window.rejectPendingTaskBtn = rejectPendingTaskBtn;
